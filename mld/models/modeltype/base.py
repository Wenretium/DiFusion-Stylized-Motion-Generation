...
class BaseModel(LightningModule):
    ...
    def allsplit_epoch_end(self, split: str, outputs):
        ...
        if split in ["val", "test"]:
            ...
            ("TM2TMetrics" in self.metrics_dict or "StyMoContMetrics" in self.metrics_dict):
    ...
    def configure_metrics(self):
        for metric in self.metrics_dict:
            if metric == "StyMoContMetrics":
                self.StyMoContMetrics = StyMoContMetrics(
                    diversity_times=30
                ...
                self.TemosMetrics = ComputeMetrics(
                ...
                try:
                    self.ClassifyMetrics = ClassifyMetrics(topk=self.cfg.METRIC.Classify_topk)
                except:
                    self.ClassifyMetrics = ClassifyMetrics()
            else:
                raise NotImplementedError(
                    f"Do not support Metric Type {metric}")
        if "TM2TMetrics" in self.metrics_dict or "UncondMetrics" in self.metrics_dict or "StyMoContMetrics" in self.metrics_dict:
