...
# TMA
from mld.tma.models.architectures.temos.textencoder.distillbert_actor import DistilbertActorAgnosticEncoder
from mld.tma.models.architectures.temos.motionencoder.actor import ActorAgnosticEncoder
...
class MLD(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        if self.condition in ["text", "text_uncond"]:
            ...
            self._get_TMA_models(cfg)
            
        try:
            # self.style_classifier = instantiate_from_config(cfg.model.style_classifier)
            from mld.models.architectures.mld_style_encoder import StyleClassification
            if cfg.METRIC.FLAG == 'TESTSET1':
    ...
    def _get_TMA_models(self, cfg):
        modelpath = '/nfs/Motion/Motion_Synthesis/OpenTMA/deps/distilbert-base-uncased'
        self.TMA_textencoder = DistilbertActorAgnosticEncoder(modelpath, num_layers=4)
        self.TMA_motionencoder = ActorAgnosticEncoder(nfeats=263, vae = True, num_layers=4)

        state_dict = torch.load('./deps/tma/humanml3d.ckpt', map_location="cpu")["state_dict"]
        ...
        self.TMA_textencoder.load_state_dict(textencoder_dict, strict=True)
        ...
        self.TMA_motionencoder.load_state_dict(motionencoder_dict, strict=True)

        self.TMA_filter_model = SentenceTransformer('sentence-transformers/paraphrase-MiniLM-L6-v2')
        
        # freeze params
        self.TMA_textencoder.eval()
        for p in self.TMA_textencoder.parameters():
            p.requires_grad = False
        self.TMA_motionencoder.eval()
        for p in self.TMA_motionencoder.parameters():
            p.requires_grad = False
        self.TMA_filter_model.eval()
        for p in self.TMA_filter_model.parameters():
            p.requires_grad = False
    ...
    def forward(self, batch, return_latent_code=False):
        texts = batch["text"]
    ...
    def t2m_eval_w_style(self, batch):
        texts = batch["text"]
        motions = batch["motion"].detach().clone()
        lengths = batch["length"]
        
        texts_cont = batch["text_cont"]
        word_embs = batch["word_embs"].detach().clone()
        pos_ohot = batch["pos_ohot"].detach().clone()
        text_lengths = batch["text_len"].detach().clone()

        style_label = batch["style"].detach().clone()
        style_label_name = batch["style_label_name"]
        ...
        motions = self.datamodule.renorm4t2m(motions)
        tma_motion = feats_rst.clone().detach()
        tma_gt = motions.clone().detach()
        ...
        # TMA
        tma_motion_emb = self.TMA_motionencoder(tma_motion, lengths).loc
        tma_gt_emb = self.TMA_motionencoder(tma_gt, lengths).loc
        tma_texts = texts[int(len(texts)/2):]
        tma_text_emb = self.TMA_textencoder(tma_texts).loc
        ...
        s2m_gt_label = style_label
        if self.cfg.TESTSET1.NUM_STYLES == 100:
        ...
        rs_set = {
            ...
            "joints_rst": joints_rst,
            "tma_texts": tma_texts,
            "tma_text_emb": tma_text_emb,
            "tma_motion_emb": tma_motion_emb,
            "tma_gt_emb": tma_gt_emb,
        }
        return rs_set
    ...
    def allsplit_step(self, split: str, batch, batch_idx):
        ...
        if split in ["val", "test"]:
            if self.condition in ['text', 'text_uncond']:
                # use t2m evaluators
                if "ClassifyMetrics" in self.metrics_dict:
                    rs_set = self.t2m_eval_w_style(batch)
                else:
            ...
            for metric in metrics_dicts:
                if metric == "MostMetrics":
                    ...
                    getattr(self, metric).update(
                        ...
                        # rs_set["tma_texts"],
                        # rs_set["tma_text_emb"],
                        # rs_set["tma_motion_emb"],
                        # rs_set["tma_gt_emb"],
                    )
                elif metric == "TemosMetrics":
                    ...
                    if "ClassifyMetrics" in self.metrics_dict: # rename
                        rs_set["lat_rm"] = rs_set["t2m_gen_emb"]
