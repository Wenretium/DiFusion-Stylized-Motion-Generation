...
class MldVae(nn.Module):
    ...
    def encode(
        ...
        # print('features.shape', features.shape) # [32, 100, 263]
        ...
        latent = dist.rsample()
        # print('latent.shape', latent.shape) # [1, 32, 256]
        return latent, dist
    ...
    def decode(self, z: Tensor, lengths: List[int]):
        ...
        if self.arch == "all_encoder":
            ...
            if self.pe_type == "actor":
                ...
                output = self.g(tgt=queries,
                                      memory=z,
                                      tgt_key_padding_mask=~mask).squeeze(0)
            elif self.pe_type == "mld":
                queries = self.query_pos_decoder(queries)
                # mem_pos = self.mem_pos_decoder(z)
                # print('vae_decoder', queries.shape, z.shape, mask.shape) # torch.Size([length, bs, ndim]) torch.Size([1, bs, ndim]) torch.Size([bs, length])
                output = self.decoder(
                    tgt=queries,
                    memory=z,
