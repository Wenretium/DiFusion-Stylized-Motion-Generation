...
class StyleClassification(nn.Module):
    def __init__(self,
        ...
        if use_temporal_atten:
            # encoder_layer_s = TransformerEncoderTemporalLayer(
            #     self.latent_dim,
            #     num_heads,
            #     ff_size,
            #     dropout,
    ...
    def forward(
        ...
        if stage == "intermediate":
            ...
            # feat = F.avg_pool1d(dist.permute(1, 2, 0), kernel_size=dist.shape[0]).permute(2, 0, 1)[0]
            if stage == "Encode":
                #[2, 64, 256]
                feat = dist[0]
                # feat = F.max_pool1d(dist.permute(1, 2, 0), kernel_size=dist.shape[0]).permute(2, 0, 1)[0]
