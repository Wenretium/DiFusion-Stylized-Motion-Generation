...
class MldDenoiser(nn.Module):
    ...
    def __init__(self,
        ...
        self.conditions_mix = ablation.CONDITIONS_MIX
        self.arch = arch
        ...
        if self.condition in ["text", "text_uncond"]:
        ...
        elif self.condition in ["text_stylecode"]:
            # text condition
            # project time from text_encoded_dim to latent_dim
            self.time_proj = Timesteps(text_encoded_dim, flip_sin_to_cos,
                                       freq_shift)
            self.time_embedding = TimestepEmbedding(text_encoded_dim,
                                                    self.latent_dim)
            self.emb_proj_text = nn.Sequential(
                    nn.ReLU(), nn.Linear(text_encoded_dim, self.latent_dim)) 
            ...
            self.emb_proj_text = nn.Sequential(
                    nn.ReLU(), nn.Linear(text_encoded_dim, self.latent_dim)) 
            self.emb_proj_stylelabel = EmbedAction(nstyles, # 直接用action的嵌入映射,能work
                                        self.latent_dim,
                                        guidance_scale=guidance_scale,
                                        guidance_uncodp=guidance_uncondp)
            # combine text and style embs
            self.mix_proj = ConditionsMixProj(self.latent_dim, 8) # n_head=8
            
        elif self.condition in ["text_stylename"]:
            self.time_proj = Timesteps(text_encoded_dim, flip_sin_to_cos,
                                       freq_shift)
            self.time_embedding = TimestepEmbedding(text_encoded_dim,
                                                    self.latent_dim)
        ...
        elif self.condition in ['style_classifier']:
            pass
    ...
    def forward(self,
        ...
        if self.condition in ["text", "text_uncond"]:
            ...
            text_emb = encoder_hidden_states["text_emb"] # [bs, 1, 768]
            text_emb = text_emb.permute(1, 0, 2)  # [num_words, bs, latent_dim]
            # textembedding projection
            if self.text_encoded_dim != self.latent_dim:
                # [1 or 2, bs, latent_dim] <= [1 or 2, bs, text_encoded_dim]
                text_emb_latent = self.emb_proj_text(text_emb) # [1, bs, 256]
            else:
                text_emb_latent = text_emb
            style_emb = encoder_hidden_states["style_emb"]
            if self.interpolate:
                style_emb1, style_emb2 = style_emb
                style_emb = self.emb_proj_stylelabel.interpolate(style_emb1, style_emb2, self.interpolate_alpha)
                # style_emb = self.emb_proj_stylelabel.interpolate(torch.full(style_emb.shape, 69).to(style_emb.device), 
                #                                                 torch.full(style_emb.shape, 42).to(style_emb.device), 0.5)
            else:
                style_emb = self.emb_proj_stylelabel(style_emb)
            style_emb_latent = style_emb
            # print('condition', time_emb.shape, text_emb_latent.shape, style_emb_latent.shape) # 都是[1, bs, 256]
            # TODO condition combine: concat is the best!
            if self.conditions_mix == 'cross_attn':
                text_emb_mixed, style_emb_mixed = self.mix_proj(sample, text_emb_latent, style_emb_latent)
                # emb_latent = torch.cat((time_emb, text_emb_mixed, style_emb_mixed), 0)
                # print('cross_attn', sample.shape, time_emb.shape, text_emb_mixed.shape, style_emb_mixed.shape) # 都是[1, bs, 256]
                sample = text_emb_mixed + style_emb_mixed
                emb_latent = time_emb
            elif self.conditions_mix == 'concat':
                emb_latent = torch.cat((time_emb, text_emb_latent, style_emb_latent), 0) # simple concat
                
        elif self.condition in ['text_stylename']:
            text_emb = encoder_hidden_states["text_emb"] # [bs, 1, 768]
            text_emb = text_emb.permute(1, 0, 2)  # [num_words, bs, latent_dim]
            style_emb = encoder_hidden_states["style_emb"]
            style_emb = style_emb.permute(1, 0, 2)  # [num_words, bs, latent_dim]
            # textembedding projection
            if self.text_encoded_dim != self.latent_dim:
                # [1 or 2, bs, latent_dim] <= [1 or 2, bs, text_encoded_dim]
                text_emb_latent = self.emb_proj_text(text_emb) # [1, bs, 256]
                style_emb_latent = self.emb_proj_text(style_emb) # [1, bs, 256]
            else:
                text_emb_latent = text_emb
                style_emb_latent = style_emb
            emb_latent = torch.cat((time_emb, text_emb_latent, style_emb_latent), 0) # simple concat
            ...
            if self.text_encoded_dim != self.latent_dim:
                ...
                text_emb_latent = text_emb
            style_emb = encoder_hidden_states["style_emb"] # [bs, 256]
            style_emb = style_emb.unsqueeze(dim=2) # [bs, 256, 1]
            style_emb_latent = style_emb.permute(2, 0, 1)  # [1, bs, 256]
            emb_latent = torch.cat((time_emb, text_emb_latent, style_emb_latent), 0) # simple concat
            
        elif self.condition in ['action']:
            # print('action', encoder_hidden_states.shape) # (64, 1)
        ...
        if self.arch == "trans_enc":
            ...
            raise TypeError("{self.arch} is not supported")
...
class EmbedAction(nn.Module):
    ...
    def interpolate(self, class1, class2, alpha):
        output1 = self.forward(class1)
        output2 = self.forward(class2)
        output_interpolate = alpha*output1 + (1-alpha)*output2

        return output_interpolate
