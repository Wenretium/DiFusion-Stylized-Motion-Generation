...
def parse_args(phase="train"):
    ...
    if phase == "demo":
        ...
        group.add_argument(
            ...
            type=str,
            required=False,
            help="input style1 of interpolate",
        )
        group.add_argument(
            "--style2",
            type=str,
            required=False,
            help="input style2 of interpolate",
        )
        group.add_argument(
            "--alpha",
            type=float,
            required=False,
            help="input interpolate alpha",
        )
        group.add_argument(
            "--content",
            type=str,
    ...
    if phase in ["train", "test"]:
        ...
        cfg.VISUALIZE = params.visualize if params.visualize is not None else cfg.VISUALIZE

        # no debug in test
        cfg.DEBUG = False if phase == "test" else cfg.DEBUG
        if phase == "test":
            cfg.DEBUG = False
            # cfg.DEVICE = [0] # 这里修改也是因为原代码CUDA_VISIBLE_DEVICES设置不成功的bug
    ...
    if phase == "demo":
        ...
        cfg.DEMO.STYLE = params.style
        cfg.DEMO.ALPHA = params.alpha
        cfg.DEMO.STYLE1 = params.style1
        cfg.DEMO.STYLE2 = params.style2
        cfg.DEMO.CONTENT = params.content
