...
def main():
    ...
    if cfg.TRAIN.RESUME:
        ...
        if os.path.exists(resume):
            # file_list = sorted(os.listdir(resume), reverse=True) # 载入旧config
    ...
    if cfg.ACCELERATOR == "gpu":
        ...
        # 如果在代码内指定CUDA_VISIBLE_DEVICES，需要在import torch之前，否则不会生效！！
    ...
    # optimizer
    metric_monitor = {
        # "Style_code_train": "stylecode/loss/train", # difusion
        "Text_align_train": "textalign/loss/train", # difusion
        "Style_classification": "classify/loss/train", # difusion
        "Inst_t2m": "instt2m/loss/train",
        "Inst_s2m": "insts2m/loss/train",
        "Triplet_train": "triplet/loss/train",
        "Total_loss_train": "total/train", # difusion
    ...
    if cfg.TRAIN.PRETRAINED:
        ...
        for k, v in state_dict.items():
            # if k.split(".")[0] not in ["style_classifier"] and k != "denoiser.emb_proj_stylelabel.action_embedding": # 做一些实验时用到，之前这里设置有点问题
            if k.split(".")[0] not in ["style_classifier"]:
                new_state_dict[k] = v

        model.load_state_dict(new_state_dict, strict=False)

    # strict load classifier model
    if cfg.TRAIN.PRETRAINED_CLASSIFIER:
        logger.info("Loading pretrain classifier from {}".format(cfg.TRAIN.PRETRAINED_CLASSIFIER))
        state_dict = torch.load(cfg.TRAIN.PRETRAINED_CLASSIFIER, map_location="cpu")
        if "state_dict" in state_dict.keys(): # 框架内写的分类器
            state_dict = state_dict["state_dict"]
            # extract encoder/decoder
            from collections import OrderedDict
            classifier_dict = OrderedDict()
            for k, v in state_dict.items():
                if k.split(".")[0] == "style_classifier":
                    name = k.replace("style_classifier.", "")
                    classifier_dict[name] = v
            model.style_classifier.load_state_dict(classifier_dict, strict=True)
        else: # SRA from SMooDi
            model.style_classifier.load_state_dict(state_dict, strict=True)
    ...
    if cfg.TRAIN.RESUME:
        ...
        # 如果resume前后的模型结构不一样,使用ckpt_path就会报错.
        # 不指定这个参数,也能载入之前的模型权重继续训练,但是epoch会从0开始计数
