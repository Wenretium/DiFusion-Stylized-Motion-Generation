...
def render_cli() -> None:
    ...
    if cfg.RENDER.INPUT_MODE.lower() == "npy":
        ...
        style_label_list = os.path.basename(cfg.RENDER.NPY).split("+")[0]
        ...
        # sort by interpolation scale
        paths = sorted(paths, key=lambda x: float(re.search(r'(\d+\.\d+)', x).group()))
        paths.reverse()
        style_label_list = [os.path.basename(path).split("+")[0] for path in paths]

    import numpy as np

    from mld.render.blender import render_multi
    ...
    if cfg.RENDER.MODE == "video":
        if os.path.exists(paths[0].replace(".npy", "_.mp4")) or os.path.exists(paths[0].replace(".npy", "_frames")):
            print(f"npy is rendered or under rendering {paths[0]}")
            # continue
    else:
        # check existed png
    ...
    if cfg.RENDER.MODE == "video":
        frames_folder = os.path.join(
            output_dir, paths[0].replace(".npy", "_frames").split('/')[-1])
        os.makedirs(frames_folder, exist_ok=True)
    else:
        frames_folder = os.path.join(
            output_dir, paths[0].replace(".npy", ".png").split('/')[-1])
    ...
    try:
        ...
        for path in paths:
            ...
            elif cfg.RENDER.JOINT_TYPE.lower() == "cmu21":
                is_mesh = mesh_detect(data)
                if not is_mesh:
                    data = data / 18
    ...
    if cfg.RENDER.MODE == "video":
        ...
        vid_path = frames_folder.replace("_frames", "_.mp4")
