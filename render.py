...
def render_cli() -> None:
    ...
    if cfg.RENDER.INPUT_MODE.lower() == "npy":
        ...
        if 'compare/video/ours' in cfg.RENDER.NPY:
            style_label = 'Default'
        else:
            style_label = os.path.basename(cfg.RENDER.NPY).split("+")[0]
    ...
    for path in paths:
        ...
        if cfg.RENDER.MODE == "video":
            print(path)
            if os.path.exists(path.replace(".npy", "_.mp4")) or os.path.exists(path.replace(".npy", "_frames")):
